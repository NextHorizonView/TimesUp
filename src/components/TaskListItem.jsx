    ...
    return (
        <View className='bg-[#F2F2F2] p-2 my-2 rounded-xl'>
            <View className='flex-row items-center'>
                {shouldDisplay ?
                    <TouchableOpacity onPress={() => setShouldDisplay(false)} className='items-center justify-center w-4 h-4 mr-3'>
                        <LottieView loop={false} ref={animationRef} style={{ width: 24, height: 24 }} source={require('../assets/tickAnimation.json')} />
                    </TouchableOpacity>
                    :
                    <TouchableOpacity onPress={() => setShouldDisplay(true)} className='w-4 h-4 mr-3 border-[1px] border-[#D2D2D2]' />
                }
                <View className='pr-8'>
